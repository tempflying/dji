...
void printCMDSession(CMDSession* cmdSession){
    printf("sessionID : %d\n", cmdSession->sessionID);
    printf("usageFlag : %d\n", cmdSession->usageFlag);
    printf("sent : %d\n", cmdSession->sent);
    printf("retry : %d\n", cmdSession->retry);
    printf("timeout : %d\n", cmdSession->timeout);
    printf("mmu : %p\n", cmdSession->mmu);
    printf("userData : %p\n", cmdSession->userData);
    printf("preSeqNum : %d\n", cmdSession->preSeqNum);
    printf("preTimestamp : %d\n", int(cmdSession->preTimestamp));
}
...
void DJI::onboardSDK::CoreAPI::appHandler(Header *header)
{
    ...
    {
        //printf("2 *************************************");
        //printCMDSession(&CMDSessionTab[header->sessionID]);
        //printHeader(header);
    ...
    for (i = 1; i < SESSION_TABLE_NUM; i++)
    {
        if (CMDSessionTab[i].usageFlag == 1)
        {
            ...
            else
            {
                ...
                //printf("dgdfgfgdfgdf   retry : %d, %d\n", (curTimestamp - CMDSessionTab[i].preTimestamp), CMDSessionTab[i].retry);
            }
        }
    }
